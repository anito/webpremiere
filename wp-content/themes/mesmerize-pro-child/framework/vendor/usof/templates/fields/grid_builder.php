			...
			if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
				...
				if ( $type == 'text' AND isset( $values['text'] ) AND ( ! empty( $values['text'] ) OR ! empty( $values['icon'] ) ) ) {
					...
					if ( isset( $values['label'] ) AND ! empty( $values['label'] ) ) {
						if ( isset( $values['add_to_cart'] ) AND $values['add_to_cart'] ) {
							$output .= us_translate( 'Add to cart', 'woocommerce' );
						} else {
...
foreach ( us_config( 'grid-settings.elements', array() ) as $type ) {
	...
	$elms_defaults['btn']['add_to_cart_label'] = us_translate( 'Add to cart', 'woocommerce' );
}
