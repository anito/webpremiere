				...
				if (!$.isNumeric(rawValue)) {
					...
					if (matches && matches[4]) {
						for (var i = 0 in this.unitsOptions) {
							if (!this.unitsOptions.hasOwnProperty(i)) continue;
...
!function($){
	...
	$usof.ButtonPreview.prototype = {
		...
		applyStyle: function() {
			...
			// Shadow
			this.$btn.css('box-shadow', '0 '+this.groupParams.getValue('shadow')/2+' '+this.groupParams.getValue('shadow')+' rgba(0,0,0,0.2)');
			// Hover class
			this.$container.usMod('hov', this.groupParams.getValue('hover'));
			// Hovered state
			this.$btn.off('mouseenter mouseleave').on('mouseenter', function() {
				this.$btn.css('box-shadow', '0 '+this.groupParams.getValue('shadow_hover')/2+' '+this.groupParams.getValue('shadow_hover')+' rgba(0,0,0,0.2)');
