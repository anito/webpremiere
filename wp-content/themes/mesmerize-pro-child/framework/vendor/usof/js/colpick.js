	...
	var colpick = function () {
		var
			...
			setAlpha = function (hsba, cal) {
				...
				colpick.alpha.css('top', parseInt($(cal).data('colpick').height * (1. - hsba.a)));
			...
			show = function (ev) {
				...
				var cal = $('#' + $(this).data('colpickId')),
					...
					calWrap = $(this).parent('.usof-color').find('.colpick_wrap')[0],
				...
				// Check whether colpick is shown
				if (this.isShown && !this.isFocus) {
					if (cal.data('colpick').onHide.apply(this, [cal.get(0)]) != false) {
						this.isFocus = false;
						this.isShown = false;
					}
				} else {
					if ((cal.data('colpick').onShow.apply(this, [cal.get(0)]) != false) && !this.isFocus) {
						this.isShown = true;
						this.isFocus = false;
					}
				}
				if (this.isFocus && ! this.isShown) {
					if (cal.data('colpick').onShow.apply(this, [cal.get(0)]) != false) {
						this.isShown = true;
						this.isFocus = false;
					}
				}
				// Hide colpick on click outside for not focus events
				if (arguments[1] != 'focus' && arguments[1]) {
					$('html').on('mousedown', function(ev){
						if (cal.data('colpick').onHide.apply(that, [cal.get(0)]) != false) {
							that.isShown = false;
							that.isFocus = false;
						}
					});
				...
				return this.each(function () {
					...
					if (!$(this).data('colpickId')) {
						...
						if (options.flat) {
							...
							cal.appendTo($(this).parent().children('.colpick_wrap'));
