	...
	if ( vc_mode() != 'page' ) {
		...
		function us_vc_param( $param_name, $param ) {
			...
			foreach ( $attributes_with_prefixes as $attribute ) {
				if ( ! empty( $param[ 'shortcode_' . $attribute ] ) ) {
...
if ( ! function_exists( 'us_icon_settings_field' ) ) {
	...
	function us_icon_settings_field( $settings, $value ) {
		...
		<div class="us-icon-desc">
			<?php echo '<a class="us-icon-set-link" href="' . $icon_sets[ $select_value ]['set_url'] . '" target="_blank">' . __( 'Enter icon name from the list', 'us' ) . '</a>. ' . sprintf( __( 'Examples: %s', 'us' ), '<span class="usof-example">star</span>, <span class="usof-example">edit</span>, <span class="usof-example">code</span>' ) ?>
		</div>
		<?php
		$result = ob_get_clean();
...
// Add script for filling inputs with font examples
...
// Add image preview for single image shortcode
...
if ( ! class_exists( 'WPBakeryShortCode_us_person' ) ) {
	class WPBakeryShortCode_us_person extends WPBakeryShortCode {
		...
		public function getImageSquereSize( $img_id, $img_size ) {
			...
			if ( isset( $exact_size['width'] ) AND (int) $exact_size['width'] !== (int) $exact_size['height'] ) {
				$img_size = (int) $exact_size['width'] > (int) $exact_size['height'] ? $exact_size['height'] . 'x' . $exact_size['height'] : $exact_size['width'] . 'x' . $exact_size['width'];
			}
