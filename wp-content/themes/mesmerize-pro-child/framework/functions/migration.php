...
class US_Migration {
	...
	public function display_migration_needed() {
		if ( $this->migration_needed_message != '' ) {
			...
			$output .= '<h2>Your website data needs to be updated to be compatible with ' . US_THEMENAME . ' ' . US_THEMEVERSION . '</h2>';
			$output .= '<p>To avoid losing website data, it\'s strongly recommended to make a full backup of your website.';
			$output .= '<br>Also don\'t edit website content and theme options before the update.</p>';
			$output .= '<p><label><input type="checkbox" name="allow_migration" id="allow_migration"> I made a full backup of my website</p>';
			$output .= '<p><input type="submit" value="Update website database" class="button" id="migration-start"></p>';
			$output .= '</div>';
		}
		echo $output;
	}
	...
	public function output_migration_needed_js() {
		...
		$output .= 'jQuery(function($){
			...
			$(".us-migration input.button").click(function(){
				...
				$(this).val("Updating...");
				$(".us-migration input.button").attr("disabled", "");
				location.assign("' . admin_url( 'edit.php?post_type=us_grid_layout' ) . '&us-migration=' . wp_create_nonce( 'us-migration' ) . '");
			});
		});';
		$output .= '</script>';
	...
	public function display_migration_completed() {
		if ( $this->migration_completed_message != '' ) {
			...
			$output .= '<p>Update to ' . US_THEMENAME . ' ' . US_THEMEVERSION . ' is completed. Now check your website. If you notice some issues, <a href="https://help.us-themes.com/' . strtolower( US_THEMENAME ) . '/tickets/" target="_blank">go to the support</a>.</p>';
			$output .= '</div>';
		}
	...
	public function migrate_content_and_meta() {
		...
		$predefined_posts_types = array( 'post', 'page', 'us_portfolio', 'us_page_block', 'us_testimonial', 'product' );
