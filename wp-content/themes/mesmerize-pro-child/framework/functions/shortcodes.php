...
class US_Shortcodes {
	...
	public function __call( $shortcode, $args ) {
		...
		if ( in_array( $element, array( 'btn', 'socials', 'image', 'gallery' ) ) ) {
...
// add_filter( 'image_size_names_choose', 'us_image_size_names_choose_add_theme_sizes' ); //TODO: maybe we do not need this anymore
...
add_action( 'init', 'us_config_shortcodes_add_grid_taxonomies_action', 11 ); //TODO: maybe we do not need this anymore
function us_config_shortcodes_add_grid_taxonomies_action() {
	add_filter( 'us_config_shortcodes', 'us_config_shortcodes_add_grid_taxonomies_filter' );
	us_config( 'shortcodes', NULL, TRUE );
}

function us_config_shortcodes_add_grid_taxonomies_filter( $config ) {
	$known_post_type_taxonomies = us_grid_available_taxonomies();
	foreach ( $known_post_type_taxonomies as $post_type => $taxonomy_slugs ) {
		$config['us_grid']['atts'][ 'filter_' . $post_type ] = NULL;
		foreach ( $taxonomy_slugs as $taxonomy_slug ) {
			$config['us_grid']['atts'][ 'taxonomy_' . $taxonomy_slug ] = NULL;
		}
	}
...
add_action( 'init', 'us_reinit_shortcodes_config' ); //TODO: maybe we do not need this anymore
function us_reinit_shortcodes_config() {
	us_config( 'shortcodes', NULL, TRUE );
}
