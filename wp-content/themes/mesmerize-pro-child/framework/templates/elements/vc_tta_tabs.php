...
if ( isset( $atts['layout'] ) AND ! empty ( $atts['layout'] ) ) {
	...
	$layout = 'ver';
	$classes .= ' navpos_' . $atts['tab_position'] . ' navwidth_' . $atts['controls_size'] . ' title_at' . $atts['c_align'];
}
...
if ( $shortcode_base != 'vc_tta_accordion' ) {
	if ( $atts['title_font'] != 'body' ) {
		$list_inline_css .= us_get_font_css( $atts['title_font'] );
	}
	if ( strpos( $atts['title_text_styles'], 'bold' ) !== FALSE ) {
		$list_inline_css .= 'font-weight:bold;';
	}
	if ( strpos( $atts['title_text_styles'], 'uppercase' ) !== FALSE ) {
		$list_inline_css .= 'text-transform:uppercase;';
	}
...
$output .= '<div class="w-tabs-list' . $list_classes . '"';
if ( ! empty ( $list_inline_css ) ) {
	$output .= ' style="' . $list_inline_css . '"';
}
$output .= '>';
