 ...
 * @param $atts ['checkbox_field'] string Checkbox field state: 'hidden' / 'required'
 ...
 * @param $atts ['icon'] string Icon name
...
if ( isset( $atts['button_fullwidth'] ) AND $atts['button_fullwidth'] ) {
	$classes = 'width_full';
} else {
...
$fields['submit'] = array(
	...
	'icon_pos' => ( ! empty( $atts['iconpos'] ) ) ? $atts['iconpos'] : 'left',
	'btn_classes' => ( ! empty( $atts['iconpos'] ) AND ! empty( $atts['icon'] ) ) ? 'icon_at' . $atts['iconpos'] : 'icon_none',
	'btn_inner_css' => ( ! empty( $atts['button_size'] ) ) ? 'font-size:' . $atts['button_size'] . ';' : '',
	'btn_size_mobiles' => ( ! empty( $atts['button_size_mobiles'] ) ) ? $atts['button_size_mobiles'] : '',
);
...
$fields['submit']['btn_classes'] .= ' us-btn-style_' . $atts['button_style'];
