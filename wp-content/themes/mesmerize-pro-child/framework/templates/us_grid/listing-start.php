 ...
 * @var $classes                      string Additional classes that will be appended to the main .w-grid container
...
$breakpoint_1_width = isset( $breakpoint_1_width ) ? $breakpoint_1_width : us_get_option( 'blog_breakpoint_1_width', 1200 );
...
$breakpoint_3_width = isset( $breakpoint_3_width ) ? $breakpoint_3_width : us_get_option( 'blog_breakpoint_3_width', 600 );
...
if ( $type != 'carousel' AND ! $is_widget ) {
	...
	if ( $columns > intval( $breakpoint_1_cols ) ) {
		...
		if ( intval( $breakpoint_1_cols ) == 1 AND ! empty( $items_gap ) ) {
			$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-list { margin: 0 }';
		}
		$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item { width:' . 100 / intval( $breakpoint_1_cols ) . '%;';
		if ( intval( $breakpoint_1_cols ) == 1 AND ! empty( $items_gap ) ) {
			$css_listing .= 'padding: 0; margin-bottom: ' . $items_gap;
		}
		$css_listing .= '}';
		if ( $post_type == 'us_portfolio' AND intval( $breakpoint_1_cols ) != 1 ) {
			$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item.size_2x1,';
			$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item.size_2x2 {';
	...
	if ( $columns > intval( $breakpoint_2_cols ) ) {
		...
		$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item { width:' . 100 / intval( $breakpoint_2_cols ) . '%;';
		if ( intval( $breakpoint_2_cols ) == 1 AND ! empty( $items_gap ) ) {
			$css_listing .= 'padding: 0; margin-bottom: ' . $items_gap;
		}
		$css_listing .= '}';
	...
	if ( $columns > intval( $breakpoint_3_cols ) ) {
		...
		$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item { width:' . 100 / intval( $breakpoint_3_cols ) . '%;';
		if ( intval( $breakpoint_3_cols ) == 1 AND ! empty( $items_gap ) ) {
			$css_listing .= 'padding: 0; margin-bottom: ' . $items_gap;
		}
		$css_listing .= '}';
...
if ( $title_size != '' ) {
	$css_listing .= '.w-grid.' . $grid_elm_id . ' .w-grid-item-elm.post_title { font-size: ' . $title_size . '}';
}
...
if ( ! empty( $item_border_radius ) ) {
	$css_layout .= 'border-radius:' . $item_border_radius . ';';
...
foreach ( $grid_layout_settings['data'] as $elm_id => $elm ) {
	...
	// Elements settings
	$css_layout .= '.w-grid.' . $grid_elm_id . ' .' . $elm_class . '{';
	if ( isset( $elm['font'] ) ) {
	...
	$css_layout .= us_prepare_inline_css(
		array(
			...
			'background-color' => isset( $elm['color_bg'] ) ? $elm['color_bg'] : '',
...
echo '<div class="w-grid' . $classes . '"' . $el_id . '>';
