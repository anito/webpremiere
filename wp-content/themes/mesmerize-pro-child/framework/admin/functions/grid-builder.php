...
function usgb_create_post_types() {
	...
	function ushb_us_grid_layout_columns_content( $column_name, $post_ID ) {
		if ( $column_name == 'used_in' ) {
			...
			if ( isset( $usof_options['shop_layout'] ) AND $usof_options['shop_layout'] == $post_ID ) {
				$used_in['woocommerce'][] = '<strong>' . __( 'Products Grid Layout', 'us' ) . '</strong>';
			}
			...
			foreach ( $used_in['woocommerce'] as $where ) {
				echo '<strong>' . $where . '</strong> (<a href="' . admin_url() . 'admin.php?page=us-theme-options#woocommerce">' . __( 'edit in Theme Options', 'us' ) . '</a>)<br>';
			}
