...
?>
	...
	.l-body:not(.rtl) .l-navigation.inv_false .to_prev {
		left: calc(<?php echo us_get_header_option( 'width' ) ?> - 13.5rem);
	}
	...
	.l-body.rtl .l-navigation.inv_false .to_next {
		right: calc(<?php echo us_get_header_option( 'width' ) ?> - 13.5rem);
	}
...
foreach ( us_get_header_elms_of_a_type( 'text' ) as $class => $param ) {
	...
	echo '.' . $class . '{';
	echo ( $param['font'] != 'body' ) ? us_get_font_css( $param['font'] ) : '';
	echo ( in_array( 'bold', $param['text_styles'] ) ) ? 'font-weight: bold;' : '';
	echo ( in_array( 'uppercase', $param['text_styles'] ) ) ? 'text-transform: uppercase;' : '';
	echo ( in_array( 'italic', $param['text_styles'] ) ) ? 'font-style: italic;' : '';
	echo ( ! empty( $param['font_size'] ) ) ? ( 'font-size:' . $param['font_size'] . ';' ) : '';
	echo ( ! empty( $param['line_height'] ) ) ? ( 'line-height:' . $param['line_height'] . ';' ) : '';
	echo ( ! $param['wrap'] ) ? 'white-space: nowrap;' : '';
	echo '}';
	if ( ! empty( $param['color'] ) ) {
		echo '.' . $class . ' .w-text-value { color:' . $param['color'] . '; }';
	}
	if ( ! empty( $param['font_size_tablets'] ) OR ! empty( $param['line_height_tablets'] ) ) {
		echo '@media ' . $tablets_query . ' {.' . $class . '{';
		echo ( ! empty( $param['font_size_tablets'] ) ) ? ( 'font-size:' . $param['font_size_tablets'] . ';' ) : '';
		echo ( ! empty( $param['line_height_tablets'] ) ) ? ( 'line-height:' . $param['line_height_tablets'] . ';' ) : '';
		echo '}}';
	}
	if ( ! empty( $param['font_size_mobiles'] ) OR ! empty( $param['line_height_mobiles'] ) ) {
		echo '@media ' . $mobiles_query . ' {.' . $class . '{';
		echo ( ! empty( $param['font_size_mobiles'] ) ) ? ( 'font-size:' . $param['font_size_mobiles'] . ';' ) : '';
		echo ( ! empty( $param['line_height_mobiles'] ) ) ? ( 'line-height:' . $param['line_height_mobiles'] . ';' ) : '';
		echo '}}';
	}
...
@media <?php echo $desktop_query ?> {
	.<?php echo $class ?> .w-btn { font-size: <?php echo $param['font_size'] ?>; }
}
@media <?php echo $tablets_query ?> {
	.<?php echo $class ?> .w-btn { font-size: <?php echo $param['font_size_tablets'] ?>; }
}
@media <?php echo $mobiles_query ?> {
	.<?php echo $class ?> .w-btn { font-size: <?php echo $param['font_size_mobiles'] ?>; }
}
<?php endforeach; ?>
